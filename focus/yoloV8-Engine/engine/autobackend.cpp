...
bool AutoBackendOnnx::fast_predict_setup() {
    if (setupCompleted_) return true;

    // Initialize input tensor shape if not already done
    if (inputTensorShape_.empty()) {
        inputTensorShape_ = getInputTensorShape();
    }

    // Allocate blob memory based on the input tensor shape
    int64_t inputTensorSize = vector_product(inputTensorShape_);
    blob_ = new float[inputTensorSize];
    inputTensorValues_.resize(inputTensorSize);

    // Create memory info for ONNX runtime
    Ort::MemoryInfo memoryInfo = Ort::MemoryInfo::CreateCpu(
        OrtAllocatorType::OrtArenaAllocator, OrtMemType::OrtMemTypeDefault);

    // Preallocate input tensor
    inputTensors_.emplace_back(Ort::Value::CreateTensor<float>(
        memoryInfo, inputTensorValues_.data(), inputTensorSize,
...
std::vector<YoloResults> AutoBackendOnnx::fast_predict(cv::Mat& image, float& conf, float& iou, float& mask_threshold, int conversionCode) {
    ...
    letterbox(image, preprocessed_img, new_shape, cv::Scalar(), auto_, scaleFill, true, getStride());
    fill_blob(preprocessed_img, blob_, inputTensorShape_);

    // Copy the preprocessed image data to the input tensor
    std::copy(blob_, blob_ + inputTensorValues_.size(), inputTensorValues_.begin());

    // Perform inference
    std::vector<Ort::Value> outputTensors = forward(inputTensors_);

    // Postprocess the results
    std::vector<YoloResults> results;
    std::unordered_map<int, std::string> names = getNames();
    int class_names_num = names.size();
...
/**
 * @brief Converts an image to a float blob in CHW format for ONNX model input.
 *
 * @param image Input image to be converted.
 * @param blob Pointer to the float array where the blob data will be stored.
 * @param inputTensorShape Shape of the input tensor, used to determine the number of channels.
 */
void AutoBackendOnnx::fill_blob(cv::Mat& image, float*& blob, std::vector<int64_t>& inputTensorShape) {

    // Create a Mat to hold the float version of the input image
    cv::Mat floatImage;

    // If input tensor shape is empty, get it using a helper function
    ...
    // Convert the image to a float type with 3 channels (CV_32FC3)
    int rtype = CV_32FC3;
    image.convertTo(floatImage, rtype, 1.0f / 255.0);

    // Allocate memory for the blob, assuming it's continuous in memory
    blob = new float[floatImage.cols * floatImage.rows * floatImage.channels()];

    // Size of the float image
    cv::Size floatImageSize{ floatImage.cols, floatImage.rows };

    // hwc -> chw (Height-Width-Channel to Channel-Height-Width)
    std::vector<cv::Mat> chw(floatImage.channels());

    // Split the float image into separate channels and store in chw vector
