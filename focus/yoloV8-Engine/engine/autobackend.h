...
public:
    ...
    virtual std::vector<YoloResults> fast_predict(cv::Mat& image, float& conf, float& iou, float& mask_threshold, int conversionCode);

    virtual bool fast_predict_setup();
...
private:
    // Reusable members for optimization
    float* blob_ = nullptr;
