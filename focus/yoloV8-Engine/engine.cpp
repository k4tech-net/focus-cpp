...
#include "engine/onnx_model_base.h"
#include "engine/autobackend.h"
...
std::pair<int, int> calculate_move(const std::vector<YoloResults>& objs, int screenWidth, int screenHeight) {
    int deltaX = 0;
    int deltaY = 0;

    // Assuming we only care about the first detection
    if (!objs.empty()) {
        const auto& res = objs.front();

        // Calculate the middle point of the bounding box
        float centerX = res.bbox.x + res.bbox.width / 2.0f;
        float centerY = res.bbox.y + res.bbox.height / 2.0f;

        // Calculate the difference from the center of the screen
        deltaX = static_cast<int>(centerX - screenWidth / 2.0f);
        deltaY = static_cast<int>(centerY - screenHeight / 2.0f);
    }

    return { deltaX, deltaY };
}

std::tuple<int, int, cv::Mat> Engine::runInference(cv::Mat img) {

	int conversion_code = cv::COLOR_BGR2RGB;
	float mask_threshold = 0.1f;
	float conf_threshold = 0.1f;
	float iou_threshold = 0.1f;

	const std::string& modelPath = "last.onnx";
	...
	std::vector<YoloResults> objs = model.fast_predict(img, conf_threshold, iou_threshold, mask_threshold, conversion_code);

    int screenWidth = img.cols;   // Number of columns in the matrix (width)
    int screenHeight = img.rows;

    auto [movex, movey] = calculate_move(objs, screenWidth, screenHeight);
	
    return { movex, movey, img };
}