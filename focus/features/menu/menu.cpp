...
void renameConfigPopup(bool& trigger, static char renameBuffer[256]) {
	...
	if (ImGui::BeginPopupModal(xorstr_("Rename Config"), NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
		int activeFileIndex = globals.filesystem.activeFileIndex.load();

		ImGui::Text(xorstr_("Rename config:"));
		ImGui::Text(xorstr_("%s"), globals.filesystem.configFiles[activeFileIndex].c_str());
		ImGui::InputText(xorstr_("New Name"), renameBuffer, IM_ARRAYSIZE(renameBuffer));
		ImGui::Separator();

		if (ImGui::Button(xorstr_("Rename"), ImVec2(120, 0))) {
			std::string oldName = globals.filesystem.configFiles[activeFileIndex];
			std::string newName = std::string(renameBuffer) + xorstr_(".focus");
			if (rename(oldName.c_str(), newName.c_str()) == 0) {
				globals.filesystem.configFiles[activeFileIndex] = newName;
...
void Menu::popup(bool& trigger, int type) {
	...
	if (ImGui::BeginPopupModal(chartype)) {
		...
		if (type == 0) {
			...
			ImGui::Text(xorstr_("Selected config to save over: %s"), globals.filesystem.configFiles[activeFileIndex].c_str());
		}
		else if (type == 2) {
			...
			ImGui::Text(xorstr_("Selected config to delete: %s"), globals.filesystem.configFiles[activeFileIndex].c_str());
		...
		if (ImGui::Button(xorstr_("Yes"), ImVec2(120, 0))) {
			if (type == 0) {
				globals.filesystem.activeFile = globals.filesystem.configFiles[activeFileIndex];
				...
				settings.saveSettings(globals.filesystem.configFiles[activeFileIndex]);
				globals.filesystem.unsavedChanges.store(false);
				globals.filesystem.activeFile = globals.filesystem.configFiles[activeFileIndex];
			}
			else if (type == 2) {
				...
				std::string fileToDelete = globals.filesystem.configFiles[activeFileIndex];
				remove(fileToDelete.c_str());
				globals.filesystem.configFiles.erase(globals.filesystem.configFiles.begin() + activeFileIndex);
				...
				if (globals.filesystem.configFiles.empty()) {
					globals.filesystem.activeFileIndex.store(-1);
				}
				else if (activeFileIndex >= globals.filesystem.configFiles.size()) {
					globals.filesystem.activeFileIndex.store(globals.filesystem.configFiles.size() - 1);
				}
			}
		...
		if (ImGui::Button(xorstr_("No"), ImVec2(120, 0))) {
			globals.initshutdown.store(false);
			trigger = false;
			ImGui::CloseCurrentPopup();
		}
...
void Menu::startupchecks_gui() {
    ...
    if (globals.startup.mouse_driver.load()) {
	...
	if (globals.startup.keyboard_driver.load()) {
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 1.0f, 0.0f, 1.0f));
	...
	if (globals.startup.marker.load()) {
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 1.0f, 0.0f, 1.0f));
...
void Menu::gui()
{
	...
	if (ImGui::BeginTabBar(xorstr_("##TabBar")))
	{
		...
		if (ImGui::BeginTabItem(xorstr_("Aim"))) {
			...
			ImGui::Spacing();
			ImGui::Spacing();
			ImGui::SeparatorText(xorstr_("Performance"));
			...
			if (editingPattern) {
				...
				if (currentWeaponData && !currentWeaponData->empty()) {
					...
					for (size_t i = 0; i < weapon.values.size(); i++) {
						...
						if (ImGui::InputFloat(xorstr_("##X"), &weapon.values[i][0], 0.1f, 1.0f, "%.2f")) {
							globals.filesystem.unsavedChanges.store(true);
							settings.activeState.weaponDataChanged = true;
						}
						ImGui::NextColumn();
						...
						// Y value
						ImGui::SetNextItemWidth(-1);
						if (ImGui::InputFloat(xorstr_("##Y"), &weapon.values[i][1], 0.1f, 1.0f, "%.2f")) {
							globals.filesystem.unsavedChanges.store(true);
							settings.activeState.weaponDataChanged = true;
						}
						ImGui::NextColumn();
		...
		if (ImGui::BeginTabItem(xorstr_("Config"))) {
			...
			if (ImGui::Button(xorstr_("Save"))) {
				...
				if (activeFileIndex >= 0 && activeFileIndex < globals.filesystem.configFiles.size()) {
					std::string selectedFile = globals.filesystem.configFiles[activeFileIndex];
	...
	globals.initshutdown.store(!inverseShutdown);

	if (globals.initshutdown.load()) {
		if (!globals.filesystem.unsavedChanges.load()) {
			globals.done.store(true);
		}
		else {
