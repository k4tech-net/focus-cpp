...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 bg_col = GetColorU32((held || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ...
    else if (*v)
    {
        ...
        if (held || hovered) {
            window->DrawList->AddRect(check_bb.Min, check_bb.Max, bg_col, style.FrameRounding);
        }
    }
    else if (!(*v)) {
        window->DrawList->AddRect(check_bb.Min, check_bb.Max, bg_col, style.FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, bg_col, style.FrameRounding);
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        ...
        window->DrawList->AddRect(bb.Min, bb.Max, col, style.FrameRounding);
    }
    if (g.NavId == id)
    ...
    RenderTextClipped(text_min + ImVec2(1, 0), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
